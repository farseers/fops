...
// @get webApiList
func WebApiList(appName, appIp, requestIp, searchUrl string, searchUseTs int64, statusCode, pageSize, pageIndex int) collections.PageList[linkTrace_clickhouse.TraceContextPO] {
	if pageSize < 1 {
		pageSize = 20
	}
	if pageIndex < 1 {
...
// TaskList Task链路追踪列表
// @get taskList
func TaskList(appName, appIp, taskName string, searchUseTs int64, pageSize, pageIndex int) collections.PageList[linkTrace_clickhouse.TraceContextPO] {
	if pageSize < 1 {
		pageSize = 20
	}
	if pageIndex < 1 {
...
// @get fScheduleList
func FScheduleList(appName, appIp, taskName string, taskGroupId, taskId, searchUseTs int64, pageSize, pageIndex int) collections.PageList[linkTrace_clickhouse.TraceContextPO] {
	if pageSize < 1 {
		pageSize = 20
	}
	if pageIndex < 1 {
...
// @get consumerList
func ConsumerList(appName, appIp, server, queueName, routingKey string, searchUseTs int64, pageSize, pageIndex int) collections.PageList[linkTrace_clickhouse.TraceContextPO] {
	if pageSize < 1 {
		pageSize = 20
	}
	if pageIndex < 1 {
