...
<!--          </el-col>-->
          <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
            <el-form-item label="任务内容" >
              <el-tag>ID：{{state.ruleForm.Task.Id}}</el-tag>
              <el-tag>名称：{{state.ruleForm.Task.Name}}</el-tag>
              <el-tag>版本：{{state.ruleForm.Task.Ver}}</el-tag>
              <el-tag>标题：{{state.ruleForm.Task.Caption}}</el-tag>
            </el-form-item>
            <el-form-item label="运行数据" >
              <el-tag>参数：{{JSON.stringify(state.ruleForm.Task.Data) }}</el-tag>
            </el-form-item>
            <el-form-item label="运行数据" >
              <el-tag>运行耗时：{{state.ruleForm.Task.RunSpeed}}</el-tag>
              <el-tag>进度：{{state.ruleForm.Task.Progress}}</el-tag>
            </el-form-item>
            <el-form-item label="任务时间" >
              <el-tag>开始时间：{{state.ruleForm.Task.StartAt}}</el-tag>
              <el-tag>运行时间：{{state.ruleForm.Task.RunAt}}</el-tag>
              <el-tag>调度时间：{{state.ruleForm.Task.SchedulerAt}}</el-tag>
              <el-tag>创建时间：{{state.ruleForm.Task.CreateAt}}</el-tag>
            </el-form-item>
            <el-form-item label="任务状态" >
              <el-tag>状态：{{getStatusDesc(state.ruleForm.Task.Status)}}</el-tag>
            </el-form-item>
            <el-form-item label="客户端信息" >
              <el-tag>客户端ID：{{state.ruleForm.Task.Client.Id}}</el-tag>
              <el-tag>客户端名称：{{state.ruleForm.Task.Client.Name}}</el-tag>
              <el-tag>客户端IP：{{state.ruleForm.Task.Client.Ip}}</el-tag>
              <el-tag>客户端端口：{{state.ruleForm.Task.Client.Port}}</el-tag>
            </el-form-item>
          </el-col>
				</el-row>
			</el-form>
