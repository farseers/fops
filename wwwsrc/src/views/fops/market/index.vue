<template>
    ...
    <el-row :gutter="20" class="home-card-one mb15">
      <el-col :sm="6" class="mb15" v-for="(v, k) in state.topCardItemList" :key="k">
        <div class="countup-card-item countup-card-item-box" :style="{ background: `var(${v.color})` }">
          <div class="countup-card-item-flex" ref="topCardItemRefs">
            ...
            <div :id="v.id" class="countup-card-item-title-num pb6"></div>
		...
		<el-row :gutter="15" style="display: none" class="home-card-one mb15">
			<el-col
				:xs="24"
				:sm="12"
				:md="12"
...
<!--							<span class="ml5 font16" :style="{ color: v.color1 }">{{ v.num2 }}%</span>-->
		...
		<el-row :gutter="15" style="display: none" class="home-card-two mb15">
		...
		<el-row :gutter="15" style="display: none"  class="home-card-three">
...
onMounted(() => {
  ...
  initCount();
  // 10秒钟刷新一次
  state.intervalId = setInterval(initCount, 10000);
});
onBeforeUnmount(()=>{
  // 在组件销毁前清除定时器，以防止内存泄漏
