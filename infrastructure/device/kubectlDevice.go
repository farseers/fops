...
func (device kubectlDevice) SetYaml(clusterName string, projectName string, yamlContent string, progress chan string, ctx context.Context) bool {
	...
	// 发布
	var exitCode = exec.RunShellContext(ctx, "kubectl apply -f "+fileName+" --kubeconfig="+configFile+" --insecure-skip-tls-verify", progress, nil, "")
	if exitCode != 0 {
		progress <- "K8S更新镜像失败。"
		return false
	}
...
func (device kubectlDevice) SetImagesByClusterName(clusterName string, clusterConfig string, projectName string, dockerImages string, k8SControllersType eumK8SControllers.Enum, progress chan string, ctx context.Context) bool {
	...
	var exitCode = exec.RunShellContext(ctx, "kubectl set image "+k8SControllersType.String()+"/"+projectName+" "+projectName+"="+dockerImages+" --kubeconfig="+configFile+" --insecure-skip-tls-verify", progress, nil, "")
	if exitCode != 0 {
		progress <- "K8S更新镜像失败。"
		return false
	}
